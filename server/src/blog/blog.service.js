...
class Service {
  async getAllArticles (limit, page) {
  ...
  async getArticlesByCategory (categoryId, limit, page) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      if (limit == null) {
        limit = 5;
  ...
  async getUserArticles (userId, limit, page) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      if (limit == null) {
        limit = 5;
  ...
  async addArticle (title, body, category, userIdByAdmin, userId, userRole) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      if (userRole === 'admin') {
  ...
  async editArticle (blogId, title, body, category, userId, userRole) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      if (userRole === 'admin') {
        const findCategoryId = await dal.selectCategoriesByTitle(dbClient, { category });
  ...
  async deleteArticle (blogId, userId, userRole) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      if (userRole === 'admin') {
  ...
  async searchArticle (searchString, limit, page) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      if (limit == null) {
        limit = 5;
  ...
  async likeArticle (userId, blogId) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
  ...
  async dislikeArticle (userId, blogId) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      const userAlreadyLiked = await dal.getUserFromLikesTable(dbClient, { userId, blogId });
      const userAlreadyDisliked = await dal.getUserFromDislikesTable(dbClient, { userId, blogId });
  ...
  async rateAuthor (userId, blogId, requestBody) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
  ...
  async sendMessageToAuthor (userId, blogId, requestBody) {
    const dbClient = await cockroachLib.dbCponnectionPool.connect();
    try {
      const authorId = ((await dal.selectUserIdByBlogId(dbClient, { blogId })).rows[0]).userId;
